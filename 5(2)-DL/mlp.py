...
class MultiLayerPerceptron(object):
    ...
    def forward_propagation(self, X):
        ...
        """
        Layer 2: Linear -> LeakyReLU(x<=0일 때 기울기는 0.01로 설정함)
        """
        h2 = np.dot(z1, W2) + b2
        z2 = leakyrelu(h2)

        """
        Layer 3: Linear -> Skip Connection(H1 + H3) -> Tanh
        """
        h3 = np.dot(z2, W3) + b3
        z3 = tanh(h3 + h1)
    ...
    def back_propagation(self, cache, X, y, L2_norm=0.0):
        ...
        """
        Layer 3 역전파
        u = h3 + h1 이므로 dh3 = du로 역전파 진행 + dh1_skip = du로 skip connection 구현
        """
        du = dz3 * (1 - z3 ** 2)
        dh3 =  du
        dh1_skip = du
        ...
        """
        Layer 2 역전파(x>0이면 1, x<=0이면 0.01)
        """
        ...
        """
        Layer 1 역전파(x>0이면 1, x<=0이면 0)
        skip connection으로 dh1_skip이 H3에서 바로 넘어오므로 dh1 = dh1_main + dh1_skip
        """
        dh1_main = dz1 * (h1 > 0).astype(float)
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, learning_rate=1e-3, L2_norm=0.0, epoch=20000, print_loss=True):
        ...
        for it in range(epoch):
            ...
            """
            순전파(line 233), loss 계산(line 234), 역전파(line 235)
            """
